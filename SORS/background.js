...
async function scanStart() {
	...
	let myListings = await fetchRetry('https://steamcommunity.com/market/mylistings/?norender=1', "json");
	...
	if (scan.buyOrders.length > 0)
		scanOrder(scan.buyOrders, 0);
}
...
async function scanOrder(orders, index) {
	...
	let sourceCode = await fetchRetry(orderHref, "text");
	...
	let itemGraph = await fetchRetry(itemGraphLink, "json");
	...
	if (sourceCode.includes('<div id="market_commodity_order_spread">')) {
		let avarageOfTwo = itemGraph.sell_order_graph.map(a => a[0]).slice(0, 4).reduce((a, b) => a + b) / 4 - 0.01;
		...
		marketPrice = marketPrice - CalculateFeeAmount(marketPrice, scan.g_rgWalletInfo.wallet_publisher_fee_percent_default).fees;
	}
	...
	if (marketPrice * (100 - params.cancelHighPercent) / 100 < orderPrice) {
		...
		let breakEvenPoint = CalculateAmountToSendForDesiredReceivedAmount(orderPrice, scan.g_rgWalletInfo.wallet_publisher_fee_percent_default).amount;
		let marketPriceWithFee = CalculateAmountToSendForDesiredReceivedAmount(marketPrice, scan.g_rgWalletInfo.wallet_publisher_fee_percent_default).amount;
...
async function fetchRetry(url, type, attempts = 3) {
...
function CalculateAmountToSendForDesiredReceivedAmount(receivedAmount, publisherFee) {
	...
	var nSteamFee = parseInt(Math.floor(Math.max(receivedAmount * parseFloat(scan.g_rgWalletInfo['wallet_fee_percent']), scan.g_rgWalletInfo['wallet_fee_minimum']) + parseInt(scan.g_rgWalletInfo['wallet_fee_base'])));
...
function CalculateFeeAmount(amount, publisherFee) {
	...
	var nEstimatedAmountOfWalletFundsReceivedByOtherParty = parseInt((amount - parseInt(scan.g_rgWalletInfo['wallet_fee_base'])) / (parseFloat(scan.g_rgWalletInfo['wallet_fee_percent']) + parseFloat(publisherFee) + 1));
